...
object Problems {
  ...
  object Problem11 extends Runnable {
    ...
    final val grid = List(8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8,
    ...
    def pos(i: Int) = (i % size, i / size)
    
    def at(x: Int, y: Int) = grid(y * size + x)
    
    def run {
      Console printf("%d",
        grid.zipWithIndex.map { case (n, i) =>
          val (x, y) = pos(i)
          var a: List[Int] = List()
          if (y + 3 < size) {
            a ::= at(x, y) * at(x, y + 1) * at(x, y + 2) * at(x, y + 3)
          }
          if (x + 3 < size) {
            a ::= at(x, y) * at(x + 1, y) * at(x + 2, y) * at(x + 3, y)
          }
          if (y + 3 < size && x + 3 < size) {
            a ::= at(x, y) * at(x + 1, y + 1) * at(x + 2, y + 2) * at(x + 3, y + 3)
            a ::= at(x + 3, y) * at(x + 2, y + 1) * at(x + 1, y + 2) * at(x, y + 3)
          }
